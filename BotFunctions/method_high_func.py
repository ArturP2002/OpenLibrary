...
from DataBaseFunctions.db_working import db_book_high_work


def high_history(bot, message: Message):
    """
...
def update_data(message: Message):
    """
    Функция telegram-бота, которая заполняет базу данных всеми найденными новыми книгами
    :param message: принятое сообщение от пользователя
    :return: new_data - список новых книг
    """
...
def books_type_high(bot, message: Message, booktype):
    """
    Функция telegram-бота, которая заносит в базу данных, какие именно книги были найдены в результате поиска
    :param bot: telegram-бот
    :param message: принятое сообщение от пользователя
    :param booktype: тип книг по результатам команд пользователя
    """
    db_book_high_work(message, booktype)
    bot.delete_message(chat_id=message.chat.id, message_id=message.message_id - 1)
    bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)


def high_answer(bot, message: Message):
    """
    Функция telegram-бота, которая отправляет пользователю сообщение от бота о результатах поиска новых книг
    :param bot: telegram-бот
    :param message: принятое сообщение от пользователя
    """
